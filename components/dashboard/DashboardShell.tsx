...
export function DashboardShell({ children, userName, userRole = "MANAGER" }: DashboardShellProps) {
  ...
  const handleRoleChange = (newRole: "DRIVER" | "MANAGER") => {
    // Drivers cannot escalate their own role
    if (isDriver) return;
    setCurrentRole(newRole);
    if (
      newRole === "DRIVER" &&
      (pathname === "/dashboard" || pathname === "/dashboard/tracking" || pathname === "/dashboard/dvir")
    ) {
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        <header className="flex h-16 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4 shadow-sm shadow-slate-200/40 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            {/* Role Switcher — visible to Managers only */}
            {!isDriver && (
            ...
            <div className="flex items-center gap-3 pl-2 border-l border-slate-200">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-semibold text-slate-600 hover:border-rose-200 hover:bg-rose-50 hover:text-rose-600 transition-colors active:scale-98"
                title="Sign out of Prado Fleet"
              >
